...
void Optimizer::interGraphOptimize(Graph *pGraph)
{
    ...
    double** cam_param;
    double** point_param;
    int camera_size = pGraph->getSupGraphSize();
    int point_size = pGraph->getSeparatorSize();
    ...
    buildProblemInterGraph(pGraph,&problem,cam_param,point_param);
    SetMinimizerOptions(&options);
    SetLinearSolver(&options);
    ...
    pGraph->updateGlobal(cam_param,point_param);
    std::cout << summary.FullReport() << "\n";
}

void
Optimizer::buildProblemInterGraph(Graph *pGraph, ceres::Problem *problem, double **cam_param, double **point_param)
{
    int camera_size = pGraph->getSupGraphSize();
    int point_size = pGraph->getSeparatorSize();

    pGraph->getInterGraphOptParameters(cam_param,point_param);
    auto edges = pGraph->getInterObservations();
    for(auto edge : edges)
    {
        auto Cam_Point = edge.first;
        auto observation = edge.second;
...
void Optimizer::optimize(Graph *pGraph, size_t submapSize)
{
